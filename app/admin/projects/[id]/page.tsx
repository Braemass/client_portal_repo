...
export default function ProjectPage({ params }: PageProps) {
  ...
  const [assets] = useState<Asset[]>(
    [
      { key: 'a', title: 'Site Overview', kind: 'image', url: 'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?w=1200&q=80' },
      { key: 'b', title: 'Closeup', kind: 'image', url: 'https://images.unsplash.com/photo-1529078155058-5d716f45d604?w=1200&q=80' },
      { key: 'c', title: 'Progress Frame', kind: 'image', url: 'https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?w=1200&q=80' },
    ]
  );

  // --- Refs to large sections (DIAGONAL QUAD page originally used this) -----
  ...
  const setLargeRef = useCallback(
    (key: string) => (el: HTMLDivElement | null) => {
      // ✅ assign but DO NOT return anything (React ref callbacks must return void)
      largeRefs.current[key] = el;
    },
    []
  );

  // Example “highlight” logic (adjust to your real state if needed)
    ...
    <main className="mx-auto max-w-6xl p-6">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-black/60 dark:text-white/70">
          This page has its ref callbacks fixed (no return value). Replace the demo
          asset list below with your real components/data.
        </p>
      </header>
        ...
        {assets.map((asset) => {
          ...
          return (
            <div
              key={`large-${asset.key}`}
              id={`asset-${asset.key}`}
              // ✅ FIX: return void from the ref callback
            ...
            >
              <div className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm dark:border-white/15 dark:bg-[#0E1B2B]">
                ...
                {asset.kind === 'image' && asset.url ? (
                  ...
                  <img src={asset.url} alt={asset.title} className="h-80 w-full object-cover" />
                ) : (
